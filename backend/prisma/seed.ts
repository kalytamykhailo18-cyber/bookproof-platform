import { PrismaClient, UserRole, Language, BookFormat, CampaignStatus, AssignmentStatus, ReviewStatus, PaymentStatus, CreditTransactionType, WalletTransactionType, KeywordResearchStatus, TargetMarket, ContentPreference, AdminRole, CouponType, CouponAppliesTo, CommissionStatus, CustomPackageStatus, PackageApprovalStatus, NotificationType, NotificationPriority, PaymentIssueType, PaymentIssueStatus, PaymentIssueAction, DisputePriority, BehaviorFlagType, BehaviorFlagStatus, IssueSeverity } from '@prisma/client';
...
async function main() {
  ...
  const packageTiers = [
    ...
    },
    {
      ...
      currency: 'USD',
      validityDays: 180,
      description: 'Great for your first book launch',
      displayOrder: 2,
      isPopular: false,
      features: JSON.stringify([
        '50 verified reviews',
    ...
    },
    {
      ...
      currency: 'USD',
      validityDays: 180,
      description: 'Ideal for building momentum',
      displayOrder: 3,
      isPopular: true,
      features: JSON.stringify([
        '100 verified reviews',
        '180 days to activate',
        '~15-20 reviews per week',
        '20% overbooking buffer included',
        'Priority support',
      ]),
    },
  ...
  // ============================================
  // 9. READER BEHAVIOR FLAGS
  // ============================================
  console.log('\n🚩 Creating reader behavior flags...');

  const behaviorFlags = [];
  const flaggedReaders = [
    createdReaders['flaggedreader@example.com'],
    createdReaders['reader1@example.com'],
    createdReaders['reader3@example.com'],
  ];

  for (const reader of flaggedReaders) {
    const numFlags = Math.floor(Math.random() * 3) + 1; // 1-3 flags per flagged reader

    for (let i = 0; i < numFlags; i++) {
      const flagTypes = Object.values(BehaviorFlagType);
      const severities = Object.values(IssueSeverity);
      const statuses = Object.values(BehaviorFlagStatus);
      ...
      const descriptions = {
        [BehaviorFlagType.FAST_REVIEW]: 'Review completed in under 30 minutes for a 300+ page book',
        [BehaviorFlagType.SIMILAR_TEXT]: 'Review text shows 85% similarity to previous reviews',
        [BehaviorFlagType.MULTIPLE_IP]: 'Multiple readers detected from same IP address within 24 hours',
        [BehaviorFlagType.DEVICE_FINGERPRINT]: 'Device fingerprint matches 3 other reader accounts',
        [BehaviorFlagType.HIGH_REJECTION_RATE]: 'Review rejection rate is 67% - significantly above platform average',
        [BehaviorFlagType.NEW_ACCOUNT_ABUSE]: 'New account immediately applied to 5+ campaigns without profile completion',
      };

      const evidenceData = {
        [BehaviorFlagType.FAST_REVIEW]: { completionTime: 28, bookPageCount: 320, averageTime: 180 },
        [BehaviorFlagType.SIMILAR_TEXT]: { similarityScore: 0.85, matchedReviews: 3 },
        [BehaviorFlagType.MULTIPLE_IP]: { ipAddress: '192.168.1.100', accountCount: 4 },
        [BehaviorFlagType.DEVICE_FINGERPRINT]: { fingerprint: 'abc123xyz', matchedAccounts: 3 },
        [BehaviorFlagType.HIGH_REJECTION_RATE]: { rejectionRate: 0.67, platformAverage: 0.15 },
        [BehaviorFlagType.NEW_ACCOUNT_ABUSE]: { campaignsApplied: 7, accountAge: 2 },
      };
      ...
      const flag = await prisma.readerBehaviorFlag.create({
        data: {
          readerProfileId: reader.readerProfile.id,
          flagType,
          severity,
          description: descriptions[flagType],
          evidenceData: JSON.stringify(evidenceData[flagType]),
          scoreImpact: scoreImpacts[severity],
          status,
          isAutoGenerated: Math.random() > 0.3, // 70% auto-generated
          investigatedBy: status !== BehaviorFlagStatus.ACTIVE ? createdAdmins['superadmin@bookproof.app'].id : null,
          investigatedAt: status !== BehaviorFlagStatus.ACTIVE ? randomPastDate(7) : null,
          investigationNotes: status !== BehaviorFlagStatus.ACTIVE
            ? 'Investigated and found to be valid concern. Monitoring ongoing.'
            : null,
          actionTaken: status === BehaviorFlagStatus.CONFIRMED ? ['WARNED', 'SUSPENDED'][Math.floor(Math.random() * 2)] : null,
          actionTakenAt: status === BehaviorFlagStatus.CONFIRMED ? randomPastDate(5) : null,
          actionTakenBy: status === BehaviorFlagStatus.CONFIRMED ? createdAdmins['admin@bookproof.app'].id : null,
          createdAt: randomPastDate(30),
        },
      });

      behaviorFlags.push(flag);
    }
  }

  console.log(`✓ Created ${behaviorFlags.length} behavior flags for ${flaggedReaders.length} readers`);
  ...
  console.log(`   - Reader Behavior Flags: ${behaviorFlags.length}`);
